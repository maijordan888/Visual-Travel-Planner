...
├── skills/                ← 「知道什麼」—— 技術知識與架構文件
│   └── travel-planner-logic/
│       └── SKILL.md       ← 核心：完整的技術棧、資料模型、元件架構、坑點紀錄
...
> **簡單記法**：Skills = 「know-how（知識）」，Workflows = 「how-to（步驟）」
...
2. **`skills/travel-planner-logic/SKILL.md`** — 建立技術背景知識（架構/資料模型/坑點）
3. **`todos/QA_REPORT.md`** — 確認目前已知問題與待辦項目
4. **對應 `workflows/*.md`** — 若需要執行特定 UI 操作，再查找對應 workflow

> 若只需要執行單一 workflow（例如使用者說「幫我加入一個景點」），可直接跳到步驟 4，但仍建議先讀 SKILL.md 的「核心數據模型」章節。

---

## 維護規範

- **架構異動後**：必須更新 `skills/travel-planner-logic/SKILL.md`
